...
@section('content')
    ...
    <h2 class="text-slate-700 font-bold p-4 ml-2 mt-4 text-3xl">Edit Data Member</h2>
    <div class="p-5">
            ...
            <form action="{{ route('updatemember') }}" method="POST">
              ...
              <div class="form-group mb-4">
                ...
                <input type="text" class="form-control
                  ...
                  id="nama" name="nama" placeholder="Nama" value="{{$member->Name}}">
              </div>
              <div class="form-group mb-4">
                <label for="" class="form-label inline-block mb-2 text-gray-700">Alamat</label>
                <input type="text" class="form-control block
                  w-full px-3 py-1.5 text-base font-normal ext-gray-700 bg-white bg-clip-padding
                  border border-solid border-gray-300 rounded transition ease-in-out m-0
                  focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                   id="alamat" name="alamat" value="{{$member->Address}}" placeholder="Alamat">
              </div>
              <div class="form-group mb-4">
                <label for="" class="form-label inline-block mb-2 text-gray-700">No.Tlp</label>
                <input type="number" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700
                  bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0
                  focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" 
                  id="no_tlp" name="no_tlp" placeholder="No.Tlp" value="{{$member->Phone}}">
              </div>
              <div class="form-group mb-4">
                ...
                <input type="number" class="form-control block
                  ...
                  border border-solid border-gray-300 rounded transition ease-in-out m-0
                  focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" 
                  id="no_hp" name="no_hp" placeholder="No.HP" value="{{$member->HP}}">
              </div>
              <div class="form-group mb-4">
                <label for="" class="form-label inline-block mb-2 text-gray-700">Maks.Pinjam</label>
                <div class="relative">
                  <select class="block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" 
                    id="maks_pinjam" name="maks_pinjam" value="{{$member->MaxLoan}}">
                    <option value="6" {{($member->MaxLoan === 6) ? 'Selected' : ''}}>6 Hari</option>
                    <option value="3" {{($member->MaxLoan === 3) ? 'Selected' : ''}}>3 Hari</option>
                  </select>
